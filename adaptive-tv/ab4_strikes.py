"""ab4_strikes.py — A/B for Strike 1 (sync flywheel) on the live signal.
...
MODES = {
    "OFF":   {},
    "FLY":   {"ATSCPLUS_FS_COAST": "3"},
    "ERAS":  {"STVT_RS": "erasure", "STVT_RS_ERASURES": "20"},
    "BOTH":  {"ATSCPLUS_FS_COAST": "3",
...
def run_phase(idx, mode):
    ...
    kill_chain()
    time.sleep(3)
    chain_log = LOGDIR / f"ab4_p{idx}_{tag}.log"
    lf = open(chain_log, "w")
...
log({"event": "ab4_armed", "note": "waiting for 5 min radio quiet"})
quiet_since = None
while True:
    if radio_quiet():
...
results = []
order = ["OFF", "FLY", "ERAS", "BOTH", "BOTH", "ERAS", "FLY", "OFF"]
...
log({"event": "ab4_verdict",
     **{f"gaps_{m.lower()}": avg(m, "gaps_min") for m in MODES},
     **{f"real_{m.lower()}": avg(m, "real_pct") for m in MODES},
     **{f"hdrs_{m.lower()}": avg(m, "hdrs_s") for m in MODES},
     "total_coasts": sum(r["coasts"] for r in results)})
start_panel()
print("=== 4-ARM STRIKES TRIAL COMPLETE ===", flush=True)
