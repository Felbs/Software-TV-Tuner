...
## The mid-day saturation hunt (12:50-14:10) — nine falsifications to one law

User reported 20 s skips (~09:40). Falsified in order: SD degradation
(44-48 MB/s healthy), UI crash-loop (0 restarts), env loss (S16 stack
verified in-proc), observer effect (persists with instruments off),
player cushion (H6: same oso on defaults), signal (MER 19.3, 0
relocks), CPU clock (2.4 GHz, performance), soft USB replug (no
change; only physical replug truly tests the dud law), rogue influxd
(pironman5's private metrics DB, 64 MB write bursts — killed it, no
change; left dead anyway, fan control unaffected).

**The actual law: saturation arithmetic.** long+S16 chain ≈302% CPU +
player + UI ≈ 93% of 4 cores; ambient jitter punctures the residual
~3.4 oso/min on ANY channel. The overnight was clean because stock EQ
left real headroom. The morning "0 oso" long+S16 reading was a lucky
3-min window. FIX SHIPPED: per-channel EQ map (lab/eq_map.json,
{34: long, default: stock}) — the heavy EQ only where measured
necessary. FOX immediately: 2238→2360 KB/s, 3.4→1.6 oso/min, load
8→5.5. Residual 1.6/min vs overnight ~0.3/min still open — Phase 4b
endurance quantifying; suspects: daytime ambient (browser/pironman
OLED), or simply needs the browser poll-rate diet (cheap-wins list).
Timeline note: the SD-yank reboot was NEVER the cause (complaint
predates it); the reboot DID gift us boot-resilient services + SD-safe
report paths.

## Evening: the "random pauses" — real root cause found, quick fix FAILED+reverted (18:00-18:45)

The all-day skip hunt's afternoon metric (position-watcher) was noisy;
the ground-truth metric = RESYNC log lines. Clean A/B on FOX:
- default cushion: **2 RESYNC / 6 min** (matches user's "sometimes random pauses")
- big cushion (30s cache): **FROZE the Pi** (decoder starvation — buffering is the WRONG direction on a saturated board)

Tried a code fix (seek_live_solo 98%->95% + settle) to break the resync
sawtooth. It made it WORSE: **32 RESYNC / 10 min**, a 13 s EOF-pause
loop. REVERTED (git revert) — hypothesis falsified, discipline held.

**Then the real disease surfaced:** with mpv paused, `ffmpeg` extractor
count = 0 and `live_solo.ts` (what mpv plays) was NOT growing while
`live.ts` chain ran full 2379 KB/s. The extractor had died; mpv played
to the frozen EOF and paused. Watch log: extractor tried cached->full->
video modes, each "produced nothing", fell to a broken mux fallback
(vid=None aid=None). Trigger: `drop_ghost_streams`/`pid_packet_counts`
samples the TAIL of live.ts; right after a retune/rotation the tail is
THIN, so FOX's real audio pids [52,53]=0x34/0x35 show <5 packets and
get FALSE-dropped as ghosts -> extractor emits nothing. My 5+ rapid
retunes (cushion A/B + fix-apply + revert-apply) triggered exactly this.

**Verified**: after ONE clean tune, left hands-off -> extractor alive,
live_solo grows 640 KB/s, 0 produced-nothing, 0 RESYNC. So the pipeline
is healthy when not churned.

**PROPER FIXES (offline, tested — NOT live-poked):**
1. Ghost-detector needs a MIN-SAMPLE guard: never trust a <N-packet
   verdict; and/or never drop a PMT-declared AUDIO pid (only truly
   optional extras). Retry once the file matures.
2. The resync watchdog treats a DEAD EXTRACTOR (live_solo not growing)
   as an mpv-seek problem. It should detect solo-file stall and restart
   the EXTRACTOR, not seek mpv into an EOF loop.
3. Rate-limit retunes / debounce: back-to-back tunes within a few
   seconds churn the pipeline.
User's TV RESTORED to the working baseline (98% seek, clean tune);
nothing left worse than this morning. Passive 25-min observation armed.

## Evening resolution (19:00-20:25): chain_doctor bug FIXED; SDR degradation ISOLATED

TWO stacked faults behind the "random pauses" / freezes:
1. **chain_doctor Windows-ism (FIXED, committed):** liveness check gated
   on process name == "python.exe"; on Linux the chain is "python3", so
   the doctor was BLIND, judged the alive chain dead, and re-tuned it ~4x
   in the first 3 min after every tune (bounded 4/hr). Self-inflicted
   lock-loss + SDR churn. Fix: match the tv_live.py cmdline token. Chain
   then survived >7 min uninterrupted (was impossible before).
2. **RSPdx degraded (HARDWARE, needs physical replug):** guardian.py ran
   a 60-min bounded-recovery proof. Even after 6 driver-service resets,
   lock held only 1.0-3.5 min (median 1.0), 20% healthy. Short+flat hold
   = not software-recoverable. Root cause = my own earlier soft-replug
   (echo 0/1 > authorized) which per pi wing memory does NOT truly reset
   the RSP — only a physical USB unplug/replug does. Guardian's
   hold-duration metric is the clean discriminator (rising=software,
   flat=hardware) — keep it.

LESSONS: (a) NEVER soft-replug the RSPdx as a diagnostic — it degrades
into short-hold lock-loss that only a physical replug clears; (b) the
port audit missed chain_doctor's python.exe name check — grep the whole
tree for '"python.exe"' / process-name assumptions; (c) confounded
experiments burned me twice (cushion test masked by overflows; then
by chain_doctor churn) — measure the SUSPECTED-fixed layer in isolation
before A/Bing the next. Reverted this session: big-cache (froze Pi),
seek-95% resync (worse). Net committed wins: per-channel EQ, chain_doctor
python3 fix. Pending after replug: verify healthy-SDR lock stability.

## The FOX-drops-at-1-min saga — RESOLVED: evening multipath, needs long EQ (21:00-22:15)

FOX (RF36) locked ~1 min then went 100% null, permanently, across EVERY
intervention: config revert to overnight-proven, physical SDR replug,
sdrplay driver reset, kiosk-browser off, full Pi REBOOT. Hours lost
chasing system/SDR causes. THE test that cracked it (should have been
first): tune a DIFFERENT channel. NBC RF34 on Antenna A held rock-steady
4+ min -> system/SDR/chain all FINE; problem is FOX-specific. FOX won't
lock at all on Antenna A (too weak; philliups/B is the only antenna that
receives RF36). Root cause: FOX developed EVENING MULTIPATH (same as
NBC's charted evening curve) that the STOCK equalizer can't hold -- it
was clean this morning so eq_map had it on stock. Fix: eq_map FOX->long;
S16 makes long real-time on the Pi. FOX then held 4+ min at 7-8% null,
playing. LESSONS: (1) split signal-vs-system with a different-channel
tune BEFORE blaming hardware -- I tunnel-visioned on the SDR (and made
the user replug for nothing); (2) a channel clean at hour X can need the
heavy EQ at hour Y -- STATIC eq_map is wrong. THE REAL FIX (next):
ADAPTIVE EQ ESCALATION -- detect a channel losing lock on stock and
auto-bump it to long (S16), demote when clean; make it a learned,
time-aware recipe like the Knob of Time. Also: autopilot should prefer
decode-HOLD over pilot strength. Browser left disabled on the Pi TV
(chain headroom); UI available from phone/PC on the LAN.

## Overnight round 2 (eq-hunt) — THE FFT EQUALIZER halves the Pi frame-drops (00:06-01:10)

Round 1 (skip_hunt) proved player-side CPU tricks can't fix a CPU
deficit (baseline 388 drops/min best; hwdec flat = Pi 5 has NO hardware
MPEG-2 decoder; isolate/renice worse). Round 2 attacked the chain's CPU
via cheaper multipath compute paths, dual-gated on HOLD-lock + drops:
- s16 (this morning's NEON port): 382 drops/min, 302% chain CPU
- **fft (STVT_EQ_FFT overlap-save): 190 drops/min, 266% CPU** — HELD lock
- fft "rrc3": 167 (RRC_SYMS=3 was OVERRIDDEN by the governor's lean
  RRC_SYMS=4 in base_env -> a no-op; 167-vs-190 = pure noise)
Applied live: FFT EQ, verified 159 drops/60s (was 288-382). Lock held
(null 8.4%, same as s16). **THE FFT overlap-save equalizer is the Pi
multipath champion — ~50% cheaper than the NEON S16 path I ported this
morning, same lock quality.** (S16 not wasted: still holds lock, helps
x86; but on the Pi, FFT wins.) Frame-drops HALVED but not zero (~2.6/s
remains) — the Pi is still CPU-bound running long-EQ + player on 4
cores; further gains need a lighter multipath EQ or accepting the floor.
TODO to make permanent: (1) governor should default long-EQ machines to
STVT_EQ_FFT on non-Windows (currently set via the Pi's start_panel.sh
only); (2) the base_env RRC_SYMS=4 override silently defeats profile
RRC_SYMS — document/fix so experiments aren't fooled; (3) adaptive-EQ
(detect multipath, escalate stock->long+FFT) remains the real endgame.
