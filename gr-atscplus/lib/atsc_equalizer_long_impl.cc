                                 ...
                                 int nsamples)
{
    ...
    // Cadence knob (2026-07-29): convergence studies need EVERY field sync so
    // an fs_err_rms curve has field resolution. STVT_EQ_TELEM_EVERY=1 for
    // that; default 8 = the historical rate, so the LINE and its cadence are
    // unchanged for every existing parser (the panel's RE_FS, quality_tuner,
    // the MER dial). Telemetry only — the output stream is unaffected.
