                                  ...
                                  int nsamples)
{
    ...
    // ── int16 NEON blocked data path (STVT_EQ_S16=1, ported from
    // pi-port-stvt b0eadbc 2026-06-11) ── register-blocked 4-outputs-
    // per-pass vmlal_s16 kernel replaces the per-symbol VOLK dispatch.
    // Q10 input / Q11 taps; ADAPTATION STAYS FLOAT. A72 kernel bench
    // 9.1 -> 21.7 Msamp/s. Checked before the FFT path: on ARM, when
    // both are enabled, S16 wins (no FFT working-set pressure).
